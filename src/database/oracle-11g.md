...
配置文件示例（目录地址 `network\admin\listener.ora`）

```xml
# listener.ora Network Configuration File: C:\app\gybd\virtual\product\12.2.0\dbhome_1\network\admin\listener.ora
...
SID_LIST_LISTENER =
  (SID_LIST =
    (SID_DESC =
      (SID_NAME = CLRExtProc)
      (ORACLE_HOME = C:\app\gybd\virtual\product\12.2.0\dbhome_1)
      (PROGRAM = extproc)
      (ENVS = "EXTPROC_DLLS=ONLY:C:\app\gybd\virtual\product\12.2.0\dbhome_1\bin\oraclr12.dll")
    )
	# 新增服务时，需要配置，并重启服务
    (SID_DESC =
      (SID_NAME = dzzzex25)
      (GLOBAL_DBNAME = dzzzex25)
      (ORACLE_HOME = C:\app\gybd\virtual\product\12.2.0\dbhome_1)
    )
  )
...
LISTENER =
  (DESCRIPTION_LIST =
    (DESCRIPTION =
      (ADDRESS = (PROTOCOL = TCP)(HOST = 172.10.16.69)(PORT = 1521))
